#include "dxlib.h"
#include "Keybord.h"
#include "SceneMgr.h"
#include "GameoverScene.h"
#include "MenuScene.h"
#include "Common.h"
#include "MyDxlib.h"

//----------------------------------------------------
//定数
//メニューの項目数
#define ITEM_NUM 2

//----------------------------------------------------

static int Menupoint;
static int SE_Select, SE_Decision;
static int F_Meiryo70;
static int gameOverX;
static int F_Meiryo30;
static MenuDeta menu[ITEM_NUM]; //メニューと構造が似ているので、メニューの構造体を使う

GameOver::GameOver() {
	//"メイリオ"  の70pt(30pt),太さ3のフォントを作成
	F_Meiryo70 = CreateFontToHandle("メイリオ", 70, 3, DX_FONTTYPE_ANTIALIASING_EDGE);
	gameOverX = CalcCenterX(WINDOW_WIDE / 2, 70, "Game Over!!");
	F_Meiryo30 = CreateFontToHandle("メイリオ", 30, 3, DX_FONTTYPE_ANTIALIASING_EDGE);
	menu[0] = { -1,250,"もう一度プレイ" };
	menu[1] = { -1,350,"メニューに戻る" };
	for (int i = 0; i < ITEM_NUM; i++) {
		menu[i].x = CalcCenterX(WINDOW_WIDE / 2, 30, menu[i].MenuName);;
	}
	Menupoint = 0;
	SE_Select = LoadSoundMem("../material/se/選択音.ogg");
	SE_Decision = LoadSoundMem("../material/se/決定音.ogg");
}

void GameOver::Update() {
	if (CursorCheck(KeybordGet(KEY_INPUT_DOWN)) ||
		CursorCheck(KeybordGet(KEY_INPUT_S))) { // 下キーかSキーが押されていたら下にカーソルを移動する
		Menupoint = (Menupoint + 1) % ITEM_NUM;
		PlaySoundMem(SE_Select, DX_PLAYTYPE_BACK);
	}
	if (CursorCheck(KeybordGet(KEY_INPUT_UP)) ||
		CursorCheck(KeybordGet(KEY_INPUT_W))) { // 上キーかWキーが押されていたら上にカーソルを移動する
		Menupoint = (Menupoint + 1) % ITEM_NUM;
		PlaySoundMem(SE_Select, DX_PLAYTYPE_BACK);
	}
	if (KeybordGet(KEY_INPUT_RETURN) == 1) {//エンターキーが押されたら
		PlaySoundMem(SE_Decision, DX_PLAYTYPE_BACK);
		eScene pointScene = (Menupoint == 0 ? eGame : eMenu);
		SetScene(pointScene); //シーンを変更する
	}
}

void GameOver::Draw() {
	DrawStringToHandle(gameOverX, 130, "Game Over!!", GetColor(255, 0, 0), F_Meiryo70);

	int red = GetColor(255, 0, 0);
	int white = GetColor(255, 255, 255);
	for (int i = 0; i < ITEM_NUM; i++) {
		if (Menupoint == i)
			DrawStringToHandle(menu[i].x - 20, menu[i].y, menu[i].MenuName, red, F_Meiryo30);
		else
			DrawStringToHandle(menu[i].x, menu[i].y, menu[i].MenuName, white, F_Meiryo30);
	}
}